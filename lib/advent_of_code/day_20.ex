defmodule AdventOfCode.Day20 do
  ...
  def run_steps({key, image, min_col, max_col, min_row, max_row, all_empty_val, all_filled_val}, n) do
    run_steps({key, image, min_col-1, max_col+1, min_row-1, max_row+1, all_empty_val, all_filled_val}, n, n)
  end
  def run_steps({_, image, _, _, _, _, _, _}, 0, _), do: image
  def run_steps({key, image, min_col, max_col, min_row, max_row, all_empty_val, all_filled_val}, n, first_step) do
    coords_to_check = for col <- min_col-1..max_col+1, row <- min_row-1..max_row+1, do: {col, row}
    current_void_val = get_current_void_val(first_step, n, all_empty_val, all_filled_val)
    output_image = Enum.reduce(coords_to_check, %{}, fn coord, output ->
      Map.put(output, coord, get_square_val(key, image, coord, current_void_val))
    end)
    run_steps({key, output_image, min_col-1, max_col+1, min_row-1, max_row+1, all_empty_val, all_filled_val}, n-1, first_step)
  end
