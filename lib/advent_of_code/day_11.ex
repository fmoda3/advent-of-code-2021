defmodule AdventOfCode.Day11 do
  ...
  def to_grid(input) do
    ...
    for col <- 0..width-1, row <- 0..height-1, into: %{}, do: {{col, row}, String.to_integer(Enum.at(Enum.at(input, row), col))}
  end
  ...
  def trigger_flashes(grid) do
    {new_grid, done} = Enum.reduce(grid, {grid, true}, fn {{col, row}, v}, {new_grid, done} ->
  ...
  def count_flashes(grid) do
    count = Enum.reduce(grid, 0, fn {_, v}, acc ->
