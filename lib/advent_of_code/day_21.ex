defmodule AdventOfCode.Day21 do
  ...
  def play_regular(players, curr_player, dice, dice_count) do
    ...
    cond do
      new_score >= 1000 -> {players, dice_count+3}
      true ->
  ...
  def play_quantum(states, curr_player, next_player, p1wins, p2wins) do
    {new_states, new_p1wins, new_p2wins} =
      Enum.reduce(states, {%{}, p1wins, p2wins}, fn {{pos_1, score_1, pos_2, score_2}, count}, {new_states, new_p1wins, new_p2wins} ->
        case curr_player do
          1 ->
            {player1_states, player_wins} = play_quantum_player(pos_1, score_1, count)
            {combine_player_1_states(player1_states, new_states, pos_2, score_2), new_p1wins + player_wins, new_p2wins}
          2 ->
            {player2_states, player_wins} = play_quantum_player(pos_2, score_2, count)
            {combine_player_2_states(player2_states, new_states, pos_1, score_1), new_p1wins, new_p2wins + player_wins}
        end
      end)
  ...
  def play_quantum_player(pos, score, count) do
    Enum.reduce(@quantum_rolls, {%{}, 0}, fn {d, dcount}, {new_states, new_wins} ->
      ...
      cond do
        new_score >= 21 -> {new_states, new_wins + add_count}
        true -> {Map.update(new_states, {new_pos, new_score}, add_count, fn x -> x + add_count end), new_wins}
      end
    end)
  end
