defmodule AdventOfCode.Day09 do
  ...
  def find_lows({grid, width, height}) do
    find_lows(0, 0, width, height, grid)
  end
  ...
  def find_lows(x, y, width, height, grid) do
    ...
    find_lows(new_x, new_y, width, height, grid) ++
      if is_low_point do
        [{x, y, curr}]
      else
        []
  ...
  def basin_size(x, y, grid, visited) do
    case Map.get(grid, {x, y}) do
      ...
      # Count current square, and check adjacent
      _ ->
        {count_left, visited} =
          if MapSet.member?(visited, {x - 1, y}) do
            {0, visited}
          else
            basin_size(x - 1, y, grid, MapSet.put(visited, {x - 1, y}))
          end
        ...
        {count_up, visited} =
          if MapSet.member?(visited, {x, y - 1}) do
            {0, visited}
          else
            basin_size(x, y - 1, grid, MapSet.put(visited, {x, y - 1}))
          end
