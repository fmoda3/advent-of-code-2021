defmodule AdventOfCode.Day15 do
  ...
  def to_grid(input, multiple) do
    ...
    grid = for col <- 0..width*multiple-1, row <- 0..height*multiple-1, into: %{} do
      val = input
  ...
  def run_dijkstra({grid, width, height}) do
    ...
    risk_grid = grid
    |> Enum.reduce(%{}, fn {coord, _}, acc -> Map.put(acc, coord, 536870911) end)
    |> Map.put({0, 0}, 0)
  ...
  def run_dijkstra(grid, width, height, risk_grid, queue) do
    ...
    case risk do
      ...
      nil -> Map.get(risk_grid, {height-1, width-1})
      # Keep going
      _ ->
        {col, row} = coord
        # Check all the neighbors
        {risk_grid, queue} = [{col+1, row}, {col-1, row}, {col, row-1}, {col, row+1}]
  ...
  def add_node({col, row}, grid, risk, risk_grid, height, width, queue) do
    ...
    if row >= 0 and row < height and col >= 0 and col < width and Map.get(risk_grid, {col, row}) > risk + Map.get(grid, {col, row}) do
